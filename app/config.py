...
class Settings:
    ...
    history_log_path: str = os.path.join(logs_dir, "translation_history.jsonl")
    ...
    translation_concurrency: int = int(os.getenv("TRANSLATION_CONCURRENCY", "4"))
    ocr_enabled: bool = os.getenv("OCR_ENABLED", "true").lower() in {"1", "true", "yes", "on"}
    ocr_dpi: int = int(os.getenv("OCR_DPI", "220"))
    ocr_language: str = os.getenv("OCR_LANGUAGE", "eng")
    ocr_min_confidence: int = int(os.getenv("OCR_MIN_CONFIDENCE", "25"))
    ocr_fallback_segment_threshold: int = int(os.getenv("OCR_FALLBACK_SEGMENT_THRESHOLD", "3"))
    render_pages_per_chunk: int = int(os.getenv("RENDER_PAGES_PER_CHUNK", "6"))
    job_timeout_translate_seconds: int = int(os.getenv("JOB_TIMEOUT_TRANSLATE_SECONDS", "2700"))
    job_timeout_render_seconds: int = int(os.getenv("JOB_TIMEOUT_RENDER_SECONDS", "3600"))
    job_timeout_save_seconds: int = int(os.getenv("JOB_TIMEOUT_SAVE_SECONDS", "900"))
